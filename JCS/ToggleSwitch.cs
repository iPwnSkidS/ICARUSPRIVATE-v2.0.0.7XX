...
namespace ShitarusPrivate.JCS
{
    ...
    public class ToggleSwitch : Control
    {
            ...
            set
            {
                if (value != _checked)
                {
                    ...
                    if (value)
                    {
                        ...
                        _animationTarget = base.Width - buttonWidth;
                        BeginAnimation(checkedValue: true);
                    }
                    else
                    {
                        _animationTarget = 0;
                        BeginAnimation(checkedValue: false);
                    }
                }
            }
        }
        ...
        internal int ButtonValue
        {
            get
            {
                if (!_animating && !_moving)
                {
                    if (_checked)
                    {
                        return base.Width - _renderer.GetButtonWidth();
                    }
                    return 0;
                }
                return _buttonValue;
        ...
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        internal bool IsButtonOnRightSide => ButtonValue >= base.Width - _renderer.GetButtonWidth();
        ...
        internal bool IsButtonMovingLeft
        {
            get
            {
                if (_animating && !IsButtonOnLeftSide)
                {
        ...
        internal bool IsButtonMovingRight
        {
            get
            {
                if (_animating && !IsButtonOnRightSide)
                {
                    return _animationResult;
                }
                return false;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (_moving)
            {
                int num = _xValue + (e.Location.X - _xOffset);
                if (num < 0)
                {
                    num = 0;
                }
            ...
            else if (e.Location.X > buttonRectangle.X + buttonRectangle.Width)
            {
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (_moving)
            {
                int num = (int)(100.0 * (double)ButtonValue / ((double)base.Width - (double)buttonWidth));
                if (_checked)
                {
            ...
            else
            {
                ...
                {
                    _buttonValue = base.Width - buttonWidth;
                    _animationTarget = 0;
                }
                else
                {
                ...
                {
                    SetValueInternal(checkedValue: false);
                }
                else if (isRightSidePressed && ToggleOnSideClick)
                {
                    SetValueInternal(checkedValue: true);
                }
                Refresh();
            }
        }
